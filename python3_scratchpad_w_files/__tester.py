...
class Tester:
    ...
    def get_all_images(self):
        r"""Search the current directory for images named _image.*(Expected|Got)(\d+).png.
           Return a list of (filename, base64contents, img_html_link, column_name, row_number) where
           img_html_link is an HTML <img> element with src='{filename}' and column_name is either
           'Expected' or 'Got', defining in which result table column the image belongs. 
           row_number is the row in the result table, 0-origin, excluding the header row.
    ...
    def test_code(self):
        ...
        files = {}
        if images:
            for (filename, base64data, img_html, column, row) in images:
                self.result_table.add_image(img_html, column, row)
