...
class TargetWorld(World):
    ...
    def __init__(self, grid_size, world_params):
        ...
        self.target_icon = world_params.get('target_icon', 'flag')
        self.labelled_cells.append((tuple(world_params['target']), 'flag'))
...
class PathWorld(World):
    """A subclass of World in the turtle is required to follow a specified
       path.
    """
    def __init__(self, grid_size, world_params):
        super().__init__(grid_size, world_params)
...
class BotController:
    ...
    def at_goal(self) -> bool:
        """Check if the robot currently satisfies the goal."""
        self.world.check_final_state()
        return self.world.fail_message() == ''
